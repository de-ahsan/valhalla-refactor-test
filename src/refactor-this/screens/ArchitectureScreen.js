...
import { ARCHITECTURE } from "../constants/general";
...
export default function ArchitectureScreen() {
  ...
  useEffect(() => {
    ...
    getImages(ARCHITECTURE, page * 3 - 2).then((response) => {
      setImagesPage1(response.data);
      setLoading(false);
    });

    getImages(ARCHITECTURE, page * 3 - 1).then((response) => {
      setImagesPage2(response.data);
      setLoading(false);
    });

    getImages(ARCHITECTURE, page * 3).then((response) => {
