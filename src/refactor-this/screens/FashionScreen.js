...
import { FASHION } from "../constants/general";
...
const FashionScreen = () => {
  ...
  useEffect(() => {
    ...
    getImages(FASHION, page * 3 - 2).then((response) => {
      setImagesPage1(response.data);
      setLoading(false);
    });

    getImages(FASHION, page * 3 - 1).then((response) => {
      setImagesPage2(response.data);
      setLoading(false);
    });

    getImages(FASHION, page * 3).then((response) => {
